...
static struct s_mem_stats {
    ...
    int setCount;
    int dontKnown;
} mem_stats_zero = { 0, 0, 0, 0, 0, 0, 0, 0, 0 }, mem_stats = { 0, 0, 0, 0, 0, 0, 0, 0, 0 };
    ...
    if (mem1_isChanged(m)) {
        ...
        if (reserveHead >= RESERVESIZE) {
            LOGPRINTF(LOG_ERROR,
                      ...
                      logSize,
                      mem_stats.getCount, mem_stats.setCount, mem_stats.reserveMovesBecauseSet,
        ...
        if (mem1_isChanged(m)) {  // hash conflict with the modified cell
            ...
            if (reserveHead >= RESERVESIZE) {
                LOGPRINTF(LOG_ERROR,
                          ...
                          " reserveMovesBecauseGet=%d mainReplaces=%d notFound=%d "
                          "mainFound=%d reserveFound=%d",
                          logSize, mem_stats.getCount, mem_stats.setCount, mem_stats.reserveMovesBecauseSet,
                          mem_stats.reserveMovesBecauseGet, mem_stats.mainReplaces,
                          mem_stats.notFound, mem_stats.mainFound, mem_stats.reserveFound);
                return -1;
            }
...
int mem_commit() {
    ...
    if (!logSize) {
        ...
        for (i = 0; i < logSize; i++) {
            ...
            mem[offset].flags &= 0xFFFC;  // reset hasReserve and changed flag for this offset (even if
    ...
    TRACEPRINTF(
        "END mem_commit() logSize=%d getCount=%d setCount=%d "
        "reserveMovesBecauseSet=%d"
        " reserveMovesBecauseGet=%d mainReplaces=%d notFound=%d "
        "mainFound=%d reserveFound=%d",
        logSize, mem_stats.getCount, mem_stats.setCount, mem_stats.reserveMovesBecauseSet,
        mem_stats.reserveMovesBecauseGet, mem_stats.mainReplaces, mem_stats.notFound,
        mem_stats.mainFound, mem_stats.reserveFound);
...
int mem_commit_is_needed() {
    ...
    // if last commit was more than 1 second ago, we need to commit
