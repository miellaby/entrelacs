...
The [Entrelacs server](EntrelacsServer.md) on top of this library allows clients to simultaneously and remotely access the Arrow Space via HTTP.
...
* _blobs_
   A _blob_ is a very long binary object whose body is stored outside the persistence file. One only stores its unique cryptographic hashed signature into the Arrow Space in order to distinguish singletons.
* _tuples_ (postponed)
...
* Jumper overloading for a children list
...
* Jumper overloading for other cell chains
