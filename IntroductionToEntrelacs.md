...
The so-called _Arrow Space_ features _orthogonal persistence_. This buzzword means that the system blurs the old computing boundaries like RAM, disks and files... The Arrow Space as a whole forms a convenient and consistent work space where every agent is alive and every piece of information is saved on the fly.
...
An incremental garbage collector algorithm works inside the Arrow Space. It spares rooted arrows and all their ancestors, but eventually recycles any storage resources. This algorithm deals with reference counters but contrary to traditional system, such a simple algorithm perfectly does the job here.
...
## ... but they need for a new programming language
...
* I guess one could design an Arrow Space prototype rapidly.
